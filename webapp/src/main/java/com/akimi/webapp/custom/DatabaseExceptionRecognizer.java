package com.akimi.webapp.custom;
...
@Qualifier("Default")
public class DatabaseExceptionRecognizer implements ExceptionRecognizer {

    static final String LOGICAL_TYPE_NAME = CausewayModuleCoreRuntime.NAMESPACE
            + ".DatabaseExceptionRecognizer";
    ...
    private Optional<Recognition> recognitionOf(final Category category,
                                                final DataAccessException ex) {
        var causeMessage = _Strings.nullToEmpty(
                ex.getMostSpecificCause().getMessage()).trim();

        var exceptionFriendlyName = _Strings.asNaturalName
                .apply(ex.getClass().getSimpleName())
                .toLowerCase();

        var friendlyMessage = String.format("%s (%s)",
